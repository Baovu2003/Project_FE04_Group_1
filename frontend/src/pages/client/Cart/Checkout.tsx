...
import { Card, Row, Col, Typography, Button, Space, message, Input, Select, Modal } from 'antd';
...
export default function Checkout() {
    ...
    const [userData, setUserData] = useState({ fullName: '', email: '', phone: '', address: '' });
    ...
    useEffect(() => {
        if (selectedDistrict) {
            const fetchWards = async () => {
                try {
                    const response: DistrictResponse = await axios.get(`https://provinces.open-api.vn/api/d/${selectedDistrict}?depth=2`);
    ...
    useEffect(() => {
        const fetchUserInfo = async () => {
            try {
                const userInfo = await get(`http://localhost:5000/user/info/${user_id}`);
                setUserData({
        ...
        if (selectedItems.length === 0) {
            ...
            const fetchCart = async () => {
                try {
                    ...
                    const filterProductInCart = data.cartItems.products.filter((product) => selectedItems.includes(product.product_id));
                    const product = products.filter((productItem) =>
                        filterProductInCart.find((item) => item.product_id === productItem._id)
                    );
                ...
                <Col span={24}>
                    <Card title={<Title level={4}>Xác nhận thanh toán.</Title>}>
                        <Card style={{ marginBottom: 16 }}>
                            <Title level={5}>Thông tin người nhận. Vui lòng kiểm tra lại thông tin địa chỉ</Title>
                            {isEditing ? (
                                ...
                                <Space direction="vertical">
                                    <Text><strong>Tên:</strong> {userData.fullName}</Text>
                                    <Text><strong>Email:</strong> {userData.email}</Text>
                                    <Text><strong>Số điện thoại:</strong> {userData.phone}</Text>
                                    <Text><strong>Địa chỉ:</strong> {userData.address}</Text>
                        ...
                        {productsInCart.length === 0 ? (
                            ...
                            productsInCart.map((product) => {
                                ...
                                return (
                                    <Card key={product._id} bordered={false} style={{ position: 'relative' }}>
                                        <Row align="middle">
                                            <Col xs={12} sm={8} md={6} lg={6}>
                                                <img
                                                    src={product.thumbnail ? `http://localhost:5000${product.thumbnail}` : 'http://localhost:5000/path-to-placeholder-image.png'}
                                                    alt={product.title}
                                                    style={{ width: '100px' }}
                                                />
                                            </Col>
                        ...
                        <Card style={{ marginTop: '20px' }}>
                            ...
                            <Row justify="space-between" style={{ marginTop: '10px' }}>
                                <Text><strong>Phương thức thanh toán</strong></Text>
                                <Select
                                    value={paymentMethod}
                            ...
                            >
                                <h6 style={{ textAlign: 'center' }}>Vui Lòng chuyển khoản đúng số tiền. Mọi lỗi về chuyển thiếu or thừa chúng tôi không hỗ trợ</h6>
                                <img
                                    src={qrCodeValue}
                                    alt="QR Code"
                                    style={{ maxWidth: '100%' }}
                                />
                                <p style={{ textAlign: "center" }}><strong>Quét mã để thanh toán</strong></p>
                                <CheckPayment totalMoney={totalAmount} txt={paymentCheckText} onPaymentSuccess={handleOrderPlacement} />
                            </Modal>
                        </Card>
                    </Card>
                </Col>
